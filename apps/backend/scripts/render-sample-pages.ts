 ...
 *
 * Run: npx tsx scripts/render-sample-pages.ts
 * Output: apps/landing/public/sample/sample-cover.png, sample-back.png
 */
...
async function main() {
	...
	const coverRaw = await renderPage(doc, 0);
	const backRaw = await renderPage(doc, 1);

	const cover = await addWatermark(coverRaw, 'SAMPLE — idpcompanion.com');
	const back = await addWatermark(backRaw, 'SAMPLE — idpcompanion.com');

	await writeFile(path.join(OUT_DIR, 'sample-cover.png'), cover);
	await writeFile(path.join(OUT_DIR, 'sample-back.png'), back);

	console.log(`✔ sample-cover.png (${(cover.length / 1024).toFixed(0)} KB)`);
	console.log(`✔ sample-back.png (${(back.length / 1024).toFixed(0)} KB)`);
