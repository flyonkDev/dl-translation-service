export type PrecheckFailReason = 'unsupportedType' | 'tooLarge';

export type PrecheckResult = { ok: true } | { ok: false; reasonKey: PrecheckFailReason };
...
export function runHeadshotPrecheck(file: File): PrecheckResult {
	...
	if (!okSize) return { ok: false, reasonKey: 'tooLarge' };
	return { ok: true };
}
