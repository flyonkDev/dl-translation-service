    ...
    <Transition name="pop">
      ...
      >
        <div class="panel">
          ...
          <nav class="steps">
            <div class="step" :class="current === 2 ? 'is-active' : 'is-done'">
              1. Select IDP
            </div>
            <div
              class="step"
              :class="current === 2 ? 'is-active' : current > 2 ? 'is-done' : ''"
            >
              2. Driver Details
            </div>
            <div class="step" :class="current === 3 ? 'is-active' : ''">
              3. Verify Identity
            </div>
            ...
            <form v-if="current === 2" @submit.prevent="onSubmitStep2">
              <div class="grid-2">
                <BaseInput
                  ...
                  required
                  v-model="firstName"
                  :error="showError(firstNameMeta) ? firstNameError : ''"
                ...
                <BaseInput
                  ...
                  required
                  v-model="lastName"
                  :error="showError(lastNameMeta) ? lastNameError : ''"
              ...
              <BaseInput
                ...
                type="email"
                v-model="email"
                :error="showError(emailMeta) ? emailError : ''"
                autocomplete="email"
              />
              ...
              <div class="fieldset">
                ...
                <div class="grid-3">
                  <BaseSelect
                    ...
                    v-model="dobDay"
                    :options="dayOptions"
                    :error="showError(dobDayMeta) ? dobDayError : ''"
                  />
                  <BaseSelect
                    id="dobMonth"
                    label="Month"
                    ...
                    :error="showError(dobMonthMeta) ? dobMonthError : ''"
                  />
                  <BaseSelect
                    id="dobYear"
                    label="Year"
                    ...
                    v-model="dobYear"
                    :options="yearOptions"
                    :error="showError(dobYearMeta) ? dobYearError : ''"
                  />
                </div>
              </div>
              ...
              <BaseSelect
                ...
                :error="showError(birthPlaceMeta) ? birthPlaceError : ''"
              />
            ...
            <form v-else @submit.prevent="onSubmitStep3">
              ...
              <div class="grid-2 rows">
                <BaseFile
                  ...
                  :error="
                    showError3 && !headshotFile ? 'Headshot is required' : ''
                  "
                />
                ...
                <div class="stack">
                  ...
                  />
                  <p class="hint small">
                    If your country is not available – Please Select Other
                  </p>
                </div>
                ...
                <BaseFile
                  ...
                  :error="
                    showError3 && !licenseFile
                      ? 'Driver’s license file is required'
                      : ''
                  "
                />
              </div>
              ...
              <div v-if="verificationResult" class="verification-result">
                ...
                <div v-if="verificationResult.hints?.length">
                  ...
                  <ul>
                    <li
                      v-for="hint in verificationResult.hints"
                      :key="hint"
                    >
                      {{ hint }}
                    </li>
              ...
              <p v-if="showError3 && !termsAccepted" class="app-error">
...
import { uploadLicense } from "@/features/upload-license/api/uploadLicenseApi";
...
const {
  value: dobDay,
  errorMessage: dobDayError,
  meta: dobDayMeta,
} = useField<string>("dobDay");
const {
  value: dobMonth,
  errorMessage: dobMonthError,
  meta: dobMonthMeta,
} = useField<string>("dobMonth");
const {
  value: dobYear,
  errorMessage: dobYearError,
  meta: dobYearMeta,
} = useField<string>("dobYear");
const {
  value: birthPlace,
  errorMessage: birthPlaceError,
  meta: birthPlaceMeta,
} = useField<string>("birthPlace");
...
const licenseCountry = ref<string>(
  store.verify?.licenseCountry ?? store.driver?.birthPlace ?? "",
);
const licenseFile = ref<File | null>(store.verify?.licenseFile ?? null);
const signatureDataUrl = ref<string>(
  store.verify?.signatureDataUrl ?? "",
);
const termsAccepted = ref<boolean>(
  store.verify?.termsAccepted ?? false,
);
...
const verificationResult = ref<VerifyLicenseResponse | null>(null);
const verificationError = ref<string | null>(null);

const verificationStatusLabel: ComputedRef<string> = computed(() => {
  const status = verificationResult.value?.status;
  if (!status) return "";
  if (status === "passed") return "Looks like a valid driver license ✅";
  if (status === "review")
    return "We need to review this manually ⚠️";
...
async function onSubmitStep3() {
  ...
  if (!ph.ok) {
    alert(`Headshot: ${ph.reason}`);
    return;
  }
  if (!pl.ok) {
    alert(`License: ${pl.reason}`);
    return;
  }

  // вызов бэкенда через наш api-слой
  try {
    isVerifying.value = true;

    const verification = await uploadLicense(licenseFile.value, {
    ...
    verificationError.value = e?.message || "Server error during verification";
  } finally {
