...
const ru: CountryPairCopy = {
	seo: {
		...
		description:
			'России нет в списке Markhoos из 52 стран ОАЭ. Российские права на кириллице одни не принимают в DXB — IDP это закон по Federal Decree 14/2024. Честный гид 2026: штрафы, Дубай vs Абу-Даби.',
		ogTitleShort: 'Россияне за рулём в ОАЭ',
		ogSubtitle: 'Нет в списке Markhoos. Кириллица. IDP — это закон.',
	},
	breadcrumbs: {
		home: 'Главная',
		current: 'Россия → ОАЭ',
	...
	hero: {
		...
		title: 'Международные права для россиян в ОАЭ: список Markhoos и кириллица',
		lead: 'Стойка Hertz в Дубае Терминал 3 в пик сезона оформляет сотни прокатов в час. Когда через стойку проезжает российский паспорт вместе с российскими правами — кириллица, без арабского, без английского — у агента в чек-листе два пункта, которые нужно закрыть до того, как что-либо ещё произойдёт. Первый: Россия не входит в список Markhoos из 52 одобренных стран ОАЭ — российские права не признаны для туристического вождения без IDP по Federal Decree-Law No. 14 от 2024 года. Второй: кириллицу не читают офицеры ДПС ОАЭ и сотрудники проката. Это не две проблемы с двумя решениями — это две причины, по которым один и тот же документ обязателен до того, как ключи окажутся у вас. Миллион российских туристов посещает Дубай каждый год. Те, кто узнаёт про Markhoos до прилёта, тратят на это две минуты.',
	...
	tldr: {
		...
		rows: [
			{
				...
				whatItDoes: 'Не действительны для туристического вождения — России нет в списке Markhoos; кириллицу не читают власти ОАЭ; крупные прокатные сети (Hertz, Avis, Sixt, Budget) в DXB/AUH/SHJ отказывают в выдаче машины; вождение грозит штрафом AED 2 000–10 000 (~$545–$2 720) по Federal Decree 14/2024.',
				cost: 'Уже есть',
				tone: 'neutral',
			},
			{
		...
		footnote: 'Что брать с собой в ОАЭ: оригинал российских прав (без цифровых копий) + IDP Companion (распечатанный) + загранпаспорт с действующей въездной визой ОАЭ + договор аренды + страховой полис на машину. IDP Companion — носить только вместе с оригиналом прав.',
	},
	...
	whyNotEnough: {
		...
		reasons: [
			{
				icon: 'ph:flag-bold',
				title: 'Причина 1: России нет в списке Markhoos',
				text: 'Инициатива Markhoos ОАЭ (обновление 2025) разрешает туристам из 52 одобренных стран водить по национальным правам без IDP — Великобритания, США, ЕС, Австралия, Япония, Южная Корея и другие. России в списке нет. По Federal Decree-Law No. 14 от 2024 года российский турист за рулём в ОАЭ без IDP едет с правами, не признанными властями — штраф с первого километра, AED 2 000–10 000 (~$545–$2 720) за первое нарушение.',
			},
			{
				icon: 'ph:translate-bold',
				title: 'Причина 2: кириллица не верифицируется',
				text: 'Даже если бы Россия была в списке Markhoos, кириллица на российских правах создаёт отдельную проблему верификации. Офицеры ДПС ОАЭ и сотрудники проката не могут прочитать, проверить или обработать документ в алфавите, которым они не пользуются. Каждое крупное прокатное агентство ОАЭ зафиксировало требование IDP для прав на кириллице как условие брони — применяется до проверки любых других условий.',
			},
			{
				icon: 'ph:scales-bold',
				title: 'Вместе: нет Markhoos + кириллица = двойное основание',
	...
	fines: {
		...
		colNote: 'Примечание',
		items: [
			{ icon: 'mdi:card-account-details-outline', label: 'Вождение без признанного IDP / документов', amount: 'AED 2 000–10 000 (~$545–$2 720)', note: 'Federal Decree-Law No. 14 от 2024 года; первое нарушение; России нет в Markhoos', severity: 'high' },
	...
	lez: {
		...
		tip: 'Практическое правило: в ОАЭ юридический вопрос (нужен ли IDP) решается до вылета через проверку Markhoos. Операционный вопрос (как отличается применение между эмиратами) решается тем, что вы ставите круиз-контроль на лимит, а не на +20. Камеры Абу-Даби — тихий генератор штрафов, превращающий один уикенд в трёхстрочный счёт.',
	},
	...
	alphabet: {
		...
		helps: [
			...
			'Закрывает разрыв по списку Markhoos (федеральный закон) И разрыв верификации (кириллица) одним документом',
	...
	howTo: {
		...
		lead: 'У ОАЭ — самое чёткое требование IDP в этом гиде для российских туристов. Две минуты онлайн до вылета снимают и проблему списка Markhoos, и разрыв верификации кириллицы.',
		...
		steps: [
			{
				...
				text: '$35 — многоязычный цифровой PDF с переводом российских прав на арабский и английский (плюс французский, испанский, немецкий, итальянский и ещё 5 из набора Женевской конвенции 1949 года). Готов за 2 минуты онлайн, действует 1–5 лет. Закрывает оба основания — исключение из Markhoos + верификация кириллицы — одним документом.',
			},
			{
				title: 'Проверьте, что в загранпаспорте действующая виза ОАЭ',
				text: 'Въездные штампы и визы ОАЭ проверяют на каждом стопе полиции вместе с водительскими документами. Россиянам — visa-on-arrival; считается дата въездного штампа. Без действующей записи о въезде ни одно прокатное агентство не оформит бронь — цепочка ломается до вопроса про Markhoos.',
			},
			{
	...
	honesty: {
		...
		helps: {
			...
			items: [
				'На каждой стойке проката — Hertz, Avis, Sixt, Budget и местные конторы в DXB, AUH и SHJ — закрывая разрыв «Markhoos + кириллица» до выдачи ключей',
		...
		pattern:
			'Что реально берут с собой подготовленные российские туристы в ОАЭ: физические российские права + IDP Companion (распечатанный) + загранпаспорт + виза ОАЭ + договор аренды + страховой полис. Переводной компаньон закрывает и проблему исключения из Markhoos, и разрыв верификации кириллицы, которые сама книжка IDP не закрывает. Две минуты подготовки, $35 — и снят документальный вопрос на каждой эмиратской прокатной стойке и проверке полиции.',
	},

	renting: {
		heading: 'Аренда машины в ОАЭ с российскими правами',
		lead: 'Прокатные политики ОАЭ единообразно строги к комбинации Markhoos + кириллица. Каждая крупная сеть применяет требование IDP для российских прав как зафиксированное условие брони.',
		chains: [
			{ name: 'Hertz UAE', policy: 'IDP обязателен для всех прав не из списка Markhoos и не на латинице. Российские права попадают в обе категории. Доступен в DXB, AUH, SHJ и городских отделениях. С 21 года; доплата для младше 25. Карта обязательна' },
	...
	outcomes: {
		...
		items: [
			...
			},
			{
				...
				text: 'Все крупные сети применяют требование Markhoos + кириллица. Бронь не действует. Вы стоите у стойки, пока очередь обходит вас, потом такси до отеля — стоимость перелёта + брони уже потрачена.',
			},
			{
				severity: 'low',
				frequency: 'Иногда',
		...
		items: [
			{
				...
				answer: 'Две причины, обе юридические. Первая: России нет в списке Markhoos из 52 одобренных стран ОАЭ — российские туристические права не признаны для вождения без IDP по федеральному закону ОАЭ. Вторая: кириллицу не читают власти ОАЭ и сотрудники проката, поэтому прокатные компании применяют требование перевода как условие брони. Оба условия применяются одновременно.',
			},
			{
				id: 2,
				question: 'Какой штраф за вождение в ОАЭ без необходимых документов?',
				answer: 'По Federal Decree-Law No. 14 от 2024 года ОАЭ вождение с правами, не признанными властями ОАЭ, влечёт штраф от AED 2 000 до AED 10 000 (~$545–$2 720) за первое нарушение. Это прямое следствие отсутствия России в списке Markhoos.',
			},
			{
				id: 3,
