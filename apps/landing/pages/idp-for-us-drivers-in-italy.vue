	...
	<article class="country-pair">
		...
		<section class="section bg-slate-50">
			<div class="container max-w-3xl">
				...
				<FaqAccordion :items="copy.faq.items" id-prefix="us-it-faq" />
			</div>
		</section>
...
const copy = computed<CountryPairCopy>(() => usItalyCopy.en);
...
const canonicalPath = computed(() => String(localePath('/idp-for-us-drivers-in-italy')));
...
defineOgImageComponent('CountryPair', {
	...
	destination: { code: 'it', name: copy.value.og.destinationName },
...
useHead(() => {
	...
	// HowTo schema — separate JSON-LD block, present only when content has howTo block.
	if (copy.value.howTo) {
		baseScripts.push({
			type: 'application/ld+json',
			children: JSON.stringify({
