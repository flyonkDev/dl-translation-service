<template>
	<article class="country-pair">
		...
		<CountryPairHero
			...
			illustration="/illustrations/us-spain-hero.svg"
		...
		<!-- Why your US license isn't enough -->
		...
		<section class="section bg-slate-50">
			<div class="container max-w-3xl">
				...
				<FaqAccordion :items="copy.faq.items" id-prefix="us-es-faq" />
			</div>
		</section>
...
import { usSpainCopy } from '~/content/country-pairs/us-spain';
import type { CountryPairCopy } from '~/content/country-pairs/russia-thailand';

// EN + ES — Spanish included to capture Hispanic-American audience and
// Latin Americans visiting heritage Spain (distinct sub-audience with commercial intent).
defineI18nRoute({
	locales: ['en', 'es'],
});
...
const copy = computed<CountryPairCopy>(
	() => usSpainCopy[locale.value as 'en' | 'es'] ?? usSpainCopy.en,
);
...
const freshnessLabel = computed(() =>
	locale.value === 'es' ? 'Última revisión:' : 'Last reviewed:',
);
const lezBadge = computed(() =>
	locale.value === 'es' ? 'Específico de España' : 'Spain-specific',
);

const siteUrl = String(config.public.siteUrl || 'https://www.idpcompanion.com').replace(/\/$/, '');
const canonicalPath = computed(() => String(localePath('/idp-for-us-drivers-in-spain')));
...
defineOgImageComponent('CountryPair', {
	...
	destination: { code: 'es', name: copy.value.og.destinationName },
