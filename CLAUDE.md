...
| `ReferenceModule` | `GET /api/countries`, `GET /api/pricing` | Countries data generated by `npm run countries:generate` |
...
```bash
npm run dev                 # db:start + watch mode, :3001 (use this)
npm run start:dev           # watch mode only (assumes DB already running)
npm run db:start            # start local portable PostgreSQL (see note below)
npm run db:stop / db:status
npm run build
npm run prisma:migrate      # prisma migrate dev
npm run prisma:generate     # RE-RUN after every schema.prisma change / pull
npm run prisma:studio
npm test
npm run countries:generate  # regenerate countries dataset
```

**Local dev database (Windows).** Backend requires PostgreSQL on `localhost:5432` at boot (`prisma.service.ts` calls `$connect` in `onModuleInit`). Dev machine runs a *portable* PostgreSQL 16.4 (no Windows service, does NOT survive reboot): binaries `D:\tools\pg16\pgsql`, data `D:\tools\pg16\data`, superuser `app_user`, db `driver_license_dev`. After reboot run `npm run db:start` (or just use `npm run dev`). Paths overridable via `IDP_PG_HOME` / `IDP_PG_DATA` ([scripts/dev-db.ps1](apps/backend/scripts/dev-db.ps1)). Common boot crashes: `P1001 Can't reach database server` → DB not started; TS errors `@prisma/client has no exported member X` → stale client, run `prisma:generate`.
